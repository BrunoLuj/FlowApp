...
export const createTier=async(req,res)=>{try{const x=await model.createTier(req.body,req.user);if(!x)return res.status(404).json({error:"Program nije pronađen."});res.status(201).json(x);}catch(e){fail(res,e,"Razinu članstva nije moguće spremiti.");}};
