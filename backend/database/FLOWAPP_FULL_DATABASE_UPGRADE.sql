...
-- Hijerarhijsko povezivanje opreme stanice s mjeriteljskim centrom
ALTER TABLE equipment_assets
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='equipment_assets_metrology_type_chk') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='equipment_assets_not_self_parent_chk') THEN
    ...
    END IF;
END $$;
CREATE INDEX IF NOT EXISTS equipment_assets_parent_idx ON equipment_assets(parent_asset_id);
CREATE INDEX IF NOT EXISTS equipment_assets_station_metrology_idx
    ON equipment_assets(station_id,metrology_type) WHERE metrology_type IS NOT NULL;
UPDATE equipment_assets SET metrology_type=CASE
    ...
    ELSE metrology_type
END WHERE metrology_type IS NULL;

-- Vozni park: vozila, registracije, servisi, PP aparati i rokovi
CREATE TABLE IF NOT EXISTS fleet_vehicles (
    id BIGSERIAL PRIMARY KEY,
    registration_number VARCHAR(40) NOT NULL UNIQUE,
    make VARCHAR(100) NOT NULL, model VARCHAR(100) NOT NULL,
    vehicle_type VARCHAR(60) NOT NULL DEFAULT 'service_van',
    vin VARCHAR(80), manufacture_year INTEGER, first_registration_date DATE,
    current_odometer INTEGER NOT NULL DEFAULT 0, fuel_type VARCHAR(40),
    assigned_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL,
    status VARCHAR(30) NOT NULL DEFAULT 'active', notes TEXT,
    created_by INTEGER REFERENCES users(id) ON DELETE SET NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT fleet_vehicles_status_chk CHECK (status IN ('active','in_service','inactive','sold')),
...
CREATE TABLE IF NOT EXISTS fleet_vehicle_records (
    ...
    record_type VARCHAR(40) NOT NULL, title VARCHAR(255) NOT NULL,
    performed_at DATE, due_date DATE, odometer INTEGER, next_odometer INTEGER,
    provider VARCHAR(255), document_number VARCHAR(120), cost NUMERIC(14,2),
    currency VARCHAR(3) NOT NULL DEFAULT 'BAM',
    status VARCHAR(30) NOT NULL DEFAULT 'active', notes TEXT,
    created_by INTEGER REFERENCES users(id) ON DELETE SET NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT fleet_vehicle_records_type_chk CHECK (record_type IN (
        'registration','insurance','technical_inspection','service',
        'fire_extinguisher','tire','road_assistance','vignette','other'
    )),
    CONSTRAINT fleet_vehicle_records_status_chk CHECK (status IN ('active','completed','cancelled'))
);
...
WHERE role.name IN ('admin','project_manager','service_manager')
  AND permission.name IN ('view_fleet','manage_fleet_vehicles','manage_fleet_records','delete_fleet_records')
  AND NOT EXISTS (SELECT 1 FROM role_permissions rp WHERE rp.role_id=role.id AND rp.permission_id=permission.id);

-- Dokumenti evidencije voznog parka i loyalty centar
ALTER TABLE entity_attachments ALTER COLUMN client_id DROP NOT NULL;
ALTER TABLE entity_attachments ADD COLUMN IF NOT EXISTS fleet_record_id BIGINT
...
CREATE INDEX IF NOT EXISTS entity_attachments_fleet_record_idx ON entity_attachments(fleet_record_id);
...
CREATE TABLE IF NOT EXISTS loyalty_programs (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'active', terms TEXT,
    created_by INTEGER REFERENCES users(id) ON DELETE SET NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS loyalty_members (
    ...
    client_id INTEGER NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
    user_id INTEGER REFERENCES users(id) ON DELETE SET NULL, member_number VARCHAR(60) NOT NULL UNIQUE,
    full_name VARCHAR(255) NOT NULL, email VARCHAR(255), phone VARCHAR(80),
    tier VARCHAR(40) NOT NULL DEFAULT 'standard',
    points_balance NUMERIC(14,2) NOT NULL DEFAULT 0, lifetime_points NUMERIC(14,2) NOT NULL DEFAULT 0,
    marketing_consent BOOLEAN NOT NULL DEFAULT FALSE, status VARCHAR(20) NOT NULL DEFAULT 'active',
...
CREATE TABLE IF NOT EXISTS loyalty_transactions (
    id BIGSERIAL PRIMARY KEY, member_id BIGINT NOT NULL REFERENCES loyalty_members(id) ON DELETE CASCADE,
    transaction_type VARCHAR(20) NOT NULL, points NUMERIC(14,2) NOT NULL, amount NUMERIC(14,2),
    reference VARCHAR(120), description TEXT, expires_at DATE,
    created_by INTEGER REFERENCES users(id) ON DELETE SET NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT loyalty_transaction_type_chk CHECK (transaction_type IN ('earn','redeem','adjustment','expire'))
);
CREATE TABLE IF NOT EXISTS loyalty_rewards (
    id BIGSERIAL PRIMARY KEY, program_id BIGINT NOT NULL REFERENCES loyalty_programs(id) ON DELETE CASCADE,
    name VARCHAR(200) NOT NULL, description TEXT, points_cost NUMERIC(14,2) NOT NULL,
    valid_from DATE, valid_until DATE, quantity_limit INTEGER, status VARCHAR(20) NOT NULL DEFAULT 'active',
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS loyalty_campaigns (
    ...
    audience_tier VARCHAR(40), bonus_points NUMERIC(14,2), starts_at TIMESTAMPTZ, ends_at TIMESTAMPTZ,
    status VARCHAR(20) NOT NULL DEFAULT 'draft', created_by INTEGER REFERENCES users(id) ON DELETE SET NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
-- Digitalna Loyalty kartica, razine, kuponi, računi i audit bodova
ALTER TABLE loyalty_members ADD COLUMN IF NOT EXISTS card_token VARCHAR(64);
UPDATE loyalty_members SET card_token=md5(random()::text || clock_timestamp()::text || id::text)
...
CREATE TABLE IF NOT EXISTS loyalty_tiers (
    ...
    color VARCHAR(30) NOT NULL DEFAULT '#7c3aed', benefits TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE(program_id,name)
);
INSERT INTO loyalty_tiers(program_id,name,min_lifetime_points,color,benefits)
SELECT lp.id,t.name,t.points,t.color,t.benefits FROM loyalty_programs lp CROSS JOIN (VALUES
...
) t(name,points,color,benefits) ON CONFLICT(program_id,name) DO NOTHING;
CREATE TABLE IF NOT EXISTS loyalty_redemptions (
    ...
    coupon_code VARCHAR(40) NOT NULL UNIQUE, points_spent NUMERIC(14,2) NOT NULL,
    status VARCHAR(20) NOT NULL DEFAULT 'active', redeemed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS loyalty_receipts (
    id BIGSERIAL PRIMARY KEY, member_id BIGINT NOT NULL REFERENCES loyalty_members(id) ON DELETE CASCADE,
    transaction_id BIGINT REFERENCES loyalty_transactions(id) ON DELETE SET NULL,
    receipt_number VARCHAR(120) NOT NULL, merchant_name VARCHAR(200),
    purchased_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), total_amount NUMERIC(14,2) NOT NULL DEFAULT 0,
    currency VARCHAR(10) NOT NULL DEFAULT 'EUR', items JSONB NOT NULL DEFAULT '[]'::jsonb,
    document_url TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE(member_id,receipt_number)
);
CREATE TABLE IF NOT EXISTS loyalty_points_audit (
    id BIGSERIAL PRIMARY KEY, member_id BIGINT NOT NULL REFERENCES loyalty_members(id) ON DELETE CASCADE,
    transaction_id BIGINT REFERENCES loyalty_transactions(id) ON DELETE SET NULL,
    action VARCHAR(30) NOT NULL, points_delta NUMERIC(14,2) NOT NULL,
    balance_before NUMERIC(14,2) NOT NULL, balance_after NUMERIC(14,2) NOT NULL,
    reason TEXT, actor_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL,
    metadata JSONB NOT NULL DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS loyalty_redemptions_member_idx ON loyalty_redemptions(member_id,redeemed_at DESC);
CREATE INDEX IF NOT EXISTS loyalty_receipts_member_idx ON loyalty_receipts(member_id,purchased_at DESC);
CREATE INDEX IF NOT EXISTS loyalty_points_audit_member_idx ON loyalty_points_audit(member_id,created_at DESC);
...
END;
$$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS loyalty_points_audit_immutable ON loyalty_points_audit;
