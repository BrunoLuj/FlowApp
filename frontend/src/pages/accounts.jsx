...
import { deleteAccount, getAccounts } from '../services/accountsServices.js';
...
const Accounts = () => {
  ...
  const filteredAccounts = accounts.filter(account => {
    const matchesName = account.company_name.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = statusFilter === 'all' || 
      (statusFilter === 'active' && account.status) || 
      (statusFilter === 'inactive' && !account.status);
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-4 mt-14 sm:ml-16">
      ...
      {permissions.includes('create_accounts') && (
        <button
        ...
        <table className="min-w-full bg-white border border-gray-300 rounded-lg shadow-md">
          ...
          <tbody>
            {filteredAccounts.map((account) => {
              const { class: statusClass } = getStatusClass(account.status);
              const { text, class: statusClass1 } = getStatusText(account.status);
              return (
                <tr 
                  key={account.id} 
                ...
                >
                  <td className="py-3 px-4 border-b">{account.company_name}</td>
                  <td className="py-3 px-4 border-b">{account.address}</td>
                  <td className="py-3 px-4 border-b">{account.idbroj}</td>
                  <td className="py-3 px-4 border-b">{account.pdvbroj}</td>
                  <td className="py-3 px-4 border-b">{account.sttn_broj}</td>
                  <td className="py-3 px-4 border-b">{account.contact_person}</td>
                  <td className="py-3 px-4 border-b">{account.phone}</td>
                  <td className="py-3 px-4 border-b">{account.email}</td>
                  ...
                  <td className="py-3 px-4 border-b">
                    {permissions.includes('update_accounts') && (
                      <button
                    ...
                    {permissions.includes('delete_accounts') && (
                      <button
