...
const Commercial = () => {
  ...
  const canManage = permissions.includes("manage_commercial");
  ...
  return <div className="min-h-screen bg-slate-100 px-4 pb-10 pt-24 sm:ml-16 sm:px-7"><div className="mx-auto max-w-7xl">
    <div className="mb-6 flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-sm font-semibold uppercase tracking-widest text-indigo-600">Komercijala</p><h1 className="mt-1 text-3xl font-bold">Ugovori i ponude</h1><p className="mt-2 text-slate-500">SLA, pokriće stanica i odobravanje ponuda.</p></div>{canManage&&<div className="flex gap-2"><button onClick={()=>editContract()} className="rounded-xl border bg-white px-4 py-3 font-semibold"><FaPlus className="mr-2 inline"/>Novi ugovor</button><button onClick={()=>editQuote()} className="rounded-xl bg-indigo-600 px-4 py-3 font-semibold text-white"><FaPlus className="mr-2 inline"/>Nova ponuda</button></div>}</div>
